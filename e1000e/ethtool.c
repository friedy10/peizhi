...
#define E1000_STAT(str, m)                                                     \
  {                                                                            \
    .stat_string = str, .type = E1000_STATS,                                   \
    .sizeof_stat = sizeof(((struct e1000_adapter *)0)->m),                     \
    .stat_offset = offsetof(struct e1000_adapter, m)                           \
  }
#define E1000_NETDEV_STAT(str, m)                                              \
  {                                                                            \
    .stat_string = str, .type = NETDEV_STATS,                                  \
...
static int e1000_get_link_ksettings(struct net_device *netdev,
  ...
  if (hw->phy.media_type == e1000_media_type_copper) {
    supported = (SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full |
                 ...
                 SUPPORTED_1000baseT_Full | SUPPORTED_Asym_Pause |
    ...
    supported =
        (SUPPORTED_1000baseT_Full | SUPPORTED_FIBRE | SUPPORTED_Autoneg);

    advertising =
        (ADVERTISED_1000baseT_Full | ADVERTISED_FIBRE | ADVERTISED_Autoneg);
  ...
  cmd->base.autoneg =
      ((hw->phy.media_type == e1000_media_type_fiber) || hw->mac.autoneg)
          ? AUTONEG_ENABLE
          : AUTONEG_DISABLE;
  ...
  lp_advertising = lpa_t | mii_lpa_to_ethtool_lpa_t(adapter->phy_regs.lpa);
...
static void e1000_get_regs(struct net_device *netdev, struct ethtool_regs *regs,
  ...
  regs->version =
      (1u << 24) | (adapter->pdev->revision << 16) | adapter->pdev->device;
...
static int e1000_get_eeprom(struct net_device *netdev,
  ...
  if (hw->nvm.type == e1000_nvm_eeprom_spi) {
    ret_val =
        e1000_read_nvm(hw, first_word, last_word - first_word + 1, eeprom_buff);
  } else {
    for (i = 0; i < last_word - first_word + 1; i++) {
      ret_val = e1000_read_nvm(hw, first_word + i, 1, &eeprom_buff[i]);
      if (ret_val)
        break;
    }
  }
  ...
  if (ret_val) {
    ...
    memset(eeprom_buff, 0xff, sizeof(u16) * (last_word - first_word + 1));
  } else {
...
static int e1000_set_eeprom(struct net_device *netdev,
  ...
  if (((eeprom->offset + eeprom->len) & 1) && (!ret_val))
    ...
    ret_val =
        e1000_read_nvm(hw, last_word, 1, &eeprom_buff[last_word - first_word]);
  ...
  ret_val =
      e1000_write_nvm(hw, first_word, last_word - first_word + 1, eeprom_buff);
   ...
   */
  if ((first_word <= NVM_CHECKSUM_REG) || (hw->mac.type == e1000_82583) ||
...
static int e1000_set_ringparam(struct net_device *netdev,
  ...
  new_rx_count = clamp_t(u32, ring->rx_pending, E1000_MIN_RXD, E1000_MAX_RXD);
  new_rx_count = ALIGN(new_rx_count, REQ_RX_DESCRIPTOR_MULTIPLE);

  new_tx_count = clamp_t(u32, ring->tx_pending, E1000_MIN_TXD, E1000_MAX_TXD);
...
static int e1000_reg_test(struct e1000_adapter *adapter, u64 *data) {
  ...
  for (i = 0; i < mac->rar_entry_count; i++) {
    ...
    REG_PATTERN_TEST_ARRAY(E1000_RA, ((i << 1) + 1), mask, 0xFFFFFFFF);
...
static void e1000_free_desc_rings(struct e1000_adapter *adapter) {
  ...
  if (tx_ring->desc && tx_ring->buffer_info) {
    for (i = 0; i < tx_ring->count; i++) {
      ...
      if (buffer_info->dma)
        dma_unmap_single(&pdev->dev, buffer_info->dma, buffer_info->length,
                         DMA_TO_DEVICE);
  ...
  if (rx_ring->desc && rx_ring->buffer_info) {
    for (i = 0; i < rx_ring->count; i++) {
      ...
      if (buffer_info->dma)
        dma_unmap_single(&pdev->dev, buffer_info->dma, 2048, DMA_FROM_DEVICE);
      dev_kfree_skb(buffer_info->skb);
    }
  }
...
static int e1000_setup_desc_rings(struct e1000_adapter *adapter) {
  ...
  tx_ring->buffer_info =
      kcalloc(tx_ring->count, sizeof(struct e1000_buffer), GFP_KERNEL);
  ...
  tx_ring->desc =
      dma_alloc_coherent(&pdev->dev, tx_ring->size, &tx_ring->dma, GFP_KERNEL);
  if (!tx_ring->desc) {
    ret_val = 2;
    goto err_nomem;
  }
  ...
  for (i = 0; i < tx_ring->count; i++) {
    ...
    tx_ring->buffer_info[i].dma =
        dma_map_single(&pdev->dev, skb->data, skb->len, DMA_TO_DEVICE);
    if (dma_mapping_error(&pdev->dev, tx_ring->buffer_info[i].dma)) {
      ret_val = 4;
      goto err_nomem;
    }
    ...
    tx_desc->lower.data |=
        cpu_to_le32(E1000_TXD_CMD_EOP | E1000_TXD_CMD_IFCS | E1000_TXD_CMD_RS);
  ...
  rx_ring->buffer_info =
      kcalloc(rx_ring->count, sizeof(struct e1000_buffer), GFP_KERNEL);
  ...
  rx_ring->desc =
      dma_alloc_coherent(&pdev->dev, rx_ring->size, &rx_ring->dma, GFP_KERNEL);
  if (!rx_ring->desc) {
    ret_val = 6;
    goto err_nomem;
  }
  ...
  ew32(RDH(0), 0);
  ew32(RDT(0), 0);
  rctl = E1000_RCTL_EN | E1000_RCTL_BAM | E1000_RCTL_SZ_2048 | E1000_RCTL_UPE |
         E1000_RCTL_MPE | E1000_RCTL_LPE | E1000_RCTL_SBP | E1000_RCTL_SECRC |
  ...
  for (i = 0; i < rx_ring->count; i++) {
    ...
    rx_ring->buffer_info[i].dma =
        dma_map_single(&pdev->dev, skb->data, 2048, DMA_FROM_DEVICE);
    if (dma_mapping_error(&pdev->dev, rx_ring->buffer_info[i].dma)) {
      ret_val = 8;
      goto err_nomem;
    }
    rx_desc = E1000_RX_DESC_EXT(*rx_ring, i);
    rx_desc->read.buffer_addr = cpu_to_le64(rx_ring->buffer_info[i].dma);
...
#define KMRNCTRLSTA_OPMODE_1GB_FD_GMII 0x0582
  ew32(KMRNCTRLSTA, (KMRNCTRLSTA_OPMODE | KMRNCTRLSTA_OPMODE_1GB_FD_GMII));
...
static int e1000_run_loopback_test(struct e1000_adapter *adapter) {
  ...
  for (j = 0; j <= lc; j++) {
    ...
    for (i = 0; i < 64; i++) {
      ...
      dma_sync_single_for_device(&pdev->dev, buffer_info->dma,
    ...
    do {
      ...
      dma_sync_single_for_cpu(&pdev->dev, buffer_info->dma, 2048,
                              DMA_FROM_DEVICE);

      ret_val = e1000_check_lbtest_frame(buffer_info->skb, 1024);
      if (!ret_val)
        good_cnt++;
      l++;
      if (l == rx_ring->count)
...
static void e1000_get_wol(struct net_device *netdev,
  ...
  wol->supported = WAKE_UCAST | WAKE_MCAST | WAKE_BCAST | WAKE_MAGIC | WAKE_PHY;
  ...
  if (adapter->flags & FLAG_NO_WAKE_UCAST) {
    ...
    if (adapter->wol & E1000_WUFC_EX)
      e_err("Interface does not support directed (unicast) frame wake-up "
            "packets\n");
  }
...
static void e1000_get_ethtool_stats(struct net_device *netdev,
  ...
  for (i = 0; i < E1000_GLOBAL_STATS_LEN; i++) {
    ...
    data[i] = (e1000_gstrings_stats[i].sizeof_stat == sizeof(u64)) ? *(u64 *)p
                                                                   : *(u32 *)p;
  }
}
